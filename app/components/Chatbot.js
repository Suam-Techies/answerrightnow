...
export default function Chatbot() {
      ...
      try {
        ...
        if (!response.ok || !response.body) {
          ...
          try {
            ...
            if (
              errorText.trim() &&
              !/<!doctype html|<html[\s>]/i.test(errorText)
            ) {
        ...
        const processLine = (line) => {
          ...
          try {
            ...
            if (typeof data.show_customer_care === "boolean")
              customerCare = data.show_customer_care;
            if (data.error) streamError = data.error;
            const chunk =
              data.response || (data.type === "chunk" ? data.content : "");
            if (chunk) {
              answer += chunk;
  ...
  useEffect(() => {
    function handleStartChatClick(event) {
      const link = event.target.closest('a[href="#chatbot"]');
      if (link) setOpen(true);
    }

    document.addEventListener("click", handleStartChatClick);
    return () => document.removeEventListener("click", handleStartChatClick);
  }, []);
    ...
    <div className={styles.root} id="chatbot">
      {open ? (
        <section
          ...
          role="dialog"
          aria-label="Answer Right Now assistant"
        >
          <header className={styles.header}>
            <div className={styles.identity}>
              ...
              <div>
                ...
                <span>
                  <b />
                  {sending ? `${assistantName} is typing...` : "Online now"}
                </span>
              </div>
            </div>
          ...
          <div className={styles.messages} aria-live="polite">
            {messages.map((message, index) => (
              <div
                className={`${styles.row} ${message.from === "user" ? styles.userRow : ""}`}
              ...
              >
                ...
                <div
                  className={`${styles.message} ${message.from === "user" ? styles.userMessage : styles.botMessage}`}
                >
                  {message.text ? (
                    message.from === "bot" ? (
            ...
            {showCustomerCare && (
              <a className={styles.care} href="tel:+18334263964">
                <span>☎</span>
                <span>
                  <strong>Talk to a human agent</strong>
                  <small>+1-833-426-3964</small>
                </span>
              </a>
            )}
            <div ref={endRef} />
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            ...
            <small>
              {sending ? "Click stop to end the response" : "Enter to send"}
            </small>
          </form>
        </section>
      ) : (
        <>
          ...
          <button
            type="button"
            className={`${styles.launcher} ${launcherArriving ? styles.arriving : ""}`}
            onClick={() => {
              setOpen(true);
